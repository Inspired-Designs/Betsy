...
app.get("/reviews/:id", async (req, res) => {
  try {
    let id = req.params;
    let data = await db.query("SELECT * FROM reviews WHERE review_id = $1", [id]);
...
app.get("/pictures", async (req, res) => {
  try {
    let data = await db.query("SELECT * FROM pictures");
...
app.get("/pictures/:id", async (req, res) => {
  try {
    ...
    let data = await db.query("SELECT FROM pictures WHERE picture_id = $1", [id]);
